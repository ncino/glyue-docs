...
{% hint style="info" %}
**Adapters** are modules in Integration Gateway that handle authentication, sending, and receiving of information with external systems. Each adapter needs some initial configuration before it can be used, which is stored in an **Adapter Config.**
{% endhint %}
...
1. Log into Integration Gateway and navigate to the **Build** page (click the 🔧 \[wrench] icon on the left-hand navbar).
...
1. Return to the Dashboard on the on main Integration Gateway interface.
